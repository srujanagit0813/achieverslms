...
const CourseDetails = () => {
  ...
  const [alert, setAlert] = useState({ open: false, severity: "success", message: "" });
     ...
     const imageUrl = course?.imageUrl ? `http://localhost:5000/uploads/${course.imageUrl}` : null;
  ...
  if (loading) return <Typography sx={{ mt: 10, textAlign: "center" }}>Loading course...</Typography>;
  if (error) return <Typography sx={{ mt: 10, textAlign: "center" }} color="error">{error}</Typography>;
  if (!course) return <Typography sx={{ mt: 10, textAlign: "center" }}>Course not found</Typography>;
  ...
  return (
    <Box sx={{ backgroundColor: "#f5f7fa", py: 6, px: { xs: 2, md: 6 }, mt: 10 }}>
      <Grid container justifyContent="center" spacing={4} direction={isMobile ? "column" : "row"} alignItems="flex-start">
        {/* Left Section */}
    ...
    borderRadius: 2,
            ...
            >
              ...
              <Typography>
                {course.lessons} Lessons ⭐ {course.rating} ({course.reviews?.length || 0})
              </Typography>
            </Box>
          </Paper>
            ...
            {tab === 0 && (
              <Box>
                {course.curriculum?.map((section, index) => (
                  ...
                  >
                    <AccordionSummary expandIcon={<ExpandMoreIcon />} sx={{ bgcolor: "#f9f9f9", borderRadius: "8px" }}>
                      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
                        <Typography fontWeight={600}>{section.section}</Typography>
                        <Chip label={section.duration} size="small" color="default" variant="outlined" />
                      </Box>
                    </AccordionSummary>
                    <AccordionDetails>
                      <List disablePadding>
                        {section.items?.map((item, idx) => (
                          ...
                          >
                            ...
                            {item.duration && (
                              <Typography variant="caption" sx={{ mr: 2, color: "gray" }}>
                                ⏱ {item.duration}
                              </Typography>
                            )}
    ...
    borderRadius: 2,
            ...
            <Box sx={{ p: 3 }}>
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
                <InfoLine label="Instructor" value={course.instructor?.name || "N/A"} />
              ...
              <Button fullWidth variant="outlined" color="info" sx={{ mt: 3 }} startIcon={"📞"}>
                +47 333 78 901
              </Button>
            </Box>
          </Paper>
